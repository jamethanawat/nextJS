...
export function DataTablePagination<TData>({
  ...
  return (
    <div className='flex flex-col gap-3 p-2 border-t border-border dark:border-white/10 sm:flex-row sm:items-center sm:justify-between'>
      ...
      <div className="text-xs font-medium justify-center flex sm:flex-1">
        Page {table.getState().pagination.pageIndex + 1} of{' '}
        {table.getPageCount()}
      </div>

      <div className='w-full sm:w-auto overflow-x-auto'>
        <div className='flex items-center gap-1 justify-start sm:justify-end min-w-max'>
          <Button
            variant="outline"
          ...
          {Array.from({ length: pageCount }).map((_, i) => {
            ...
            if (i === 1 || i === pageCount - 2) {
              return <span key={i} className="px-1 text-xs">...</span>
            }
            return null
          })}
          <Button
